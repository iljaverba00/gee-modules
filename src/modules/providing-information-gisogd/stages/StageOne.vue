<template>
  ...
  >
    <template #before>
      ...
      />
      <div class="providing-information__stage-one-card">
      ...
      >
        <template #before>
          <this-header
            ...
            @update:splitter="splitter = $event"
          />
          <div class="providing-information__stage-one-files">
            <FilesList :data="props.files" />
          </div>
        </template>
        <template #after>
          <div class="providing-information__stage-one-actions">
            <q-btn
              class="providing-information__stage-one-btn"
              push
              color="primary"
...
import type { IFile, ICardRow, IReasonRefusal } from '../ProvidingInformationGisogdTypes'
...
const emit = defineEmits(['loadFirstStep', 'clearFirstStep', 'rejectFirstStep', 'nextStep'])
...
const rejection = () => {
  ...
  emit('rejectFirstStep', reasonRefusalSelect.value)
}
</script>

<style>
.providing-information__stage-one-card {
  height: calc(100% - 36px);
  overflow-y: auto;
}

.providing-information__stage-one-files {
  height: calc(100% - 36px);
  overflow-y: auto;
}

.providing-information__stage-one-actions {
  padding: 8px;
  display: flex;
  justify-content: space-between;
}

.providing-information__stage-one-btn {
  width: calc(50% - 4px);
}

.providing-information__stage-one-btn span {
  white-space: nowrap;
  overflow-x: hidden;
